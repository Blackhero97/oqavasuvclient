...
const ConsumersPage = () => {
  ...
  const loadConsumers = async (showLoadingIndicator = true) => {
    try {
      if (showLoadingIndicator) setLoading(true);
      console.log("📚 [ENHANCED] Istamolchilarni yuklash...");

      // GET /api/consumers endpoint'dan foydalanish - attendance bilan birga keladi
      const response = await axios.get(`${API_URL}/consumers`);
      const consumersFromApi = response.data || [];

      console.log(
        "🔍 [ENHANCED] API javobidagi Istamolchilar:",
        consumersFromApi.length,
        "ta"
      );

      const consumersData = consumersFromApi.map((student) => ({
        id: student._id,
        name: student.name || "Ism ko'rsatilmagan",
        class:
          student.className ||
          student.class ||
          student.department?.replace(/Istamolchi/gi, "").trim() ||
          "Sinf ko'rsatilmagan",
        age: student.age || "-",
        phone: student.phone || "-",
        email: student.email || "-",
      ...
      console.log("💾 [ENHANCED] Formatted consumers:", consumersData);
      setConsumers(consumersData);

      // Attendance data already included from API, no need for separate calls
      const attendanceMap = {};
      consumersData.forEach((student) => {
      ...
      console.error("❌ [ENHANCED] Error loading consumers:", error);
      toast.error("Istamolchilarni yuklashda xato");
    } finally {
      setLoading(false);
    }
  };

  // Attendance data for consumers
  ...
  const filteredconsumers = consumers.filter((student) => {
    const matchesSearch =
  ...
  const handleDeleteStudent = async (studentId) => {
    if (window.confirm("Istamolchini o'chirishni xohlaysizmi?")) {
      try {
        ...
        toast.success("Istamolchi o'chirildi");
        loadConsumers(false); // Refresh list
      } catch (error) {
        console.error("❌ [ENHANCED] Error deleting student:", error);
        toast.error(
          error.response?.data?.error || "Istamolchini o'chirishda xato"
        );
      }
    }
  };
    ...
    if (modalMode === "add") {
      try {
        ...
        const newStudentData = {
          ...
          department: `Istamolchi - ${formData.class}`,
          role: "student",
        ...
        toast.success("Yangi Istamolchi qo'shildi");
        setShowModal(false);
        loadConsumers(false); // Refresh list
      } catch (error) {
        console.error("❌ [ENHANCED] Error adding student:", error);
        toast.error(error.response?.data?.error || "Istamolchi qo'shishda xato");
      }
    } else {
      try {
        console.log(
        ...
        const response = await axios.put(
          `${API_URL}/consumers/${currentStudent.id}`,
          updateData
        );
        console.log("✅ [ENHANCED] Student updated:", response.data);

        toast.success("Istamolchi ma'lumotlari yangilandi");
        setShowModal(false);
        loadConsumers(false); // Refresh list
      } catch (error) {
  ...
  const handleExportToExcel = () => {
    try {
      // Istamolchilarni sinf bo'yicha guruhlash
      const consumersByClass = {};

      consumers.forEach((student) => {
        const className = student.class || "Sinf ko'rsatilmagan";
        if (!consumersByClass[className]) {
          consumersByClass[className] = [];
        }
        consumersByClass[className].push(student);
      });
      ...
      const summaryData = [
        ["BM Maktab - Istamolchilar Ro'yxati"],
        ["Sana:", new Date().toLocaleDateString("uz-UZ")],
        ["Jami Istamolchilar:", consumers.length],
        [""],
        ["Sinflar bo'yicha taqsimot:"],
      ];

      Object.keys(consumersByClass).sort().forEach((className) => {
        summaryData.push([className, consumersByClass[className].length + " ta Istamolchi"]);
      });
      ...
      let index = 1;
      Object.keys(consumersByClass).sort().forEach((className) => {
        consumersByClass[className].forEach((student) => {
          allconsumersData.push([
            index++,
            student.name || "-",
            student.class || "-",
            student.age || "-",
      ...
      const allconsumersSheet = XLSX.utils.aoa_to_sheet(allconsumersData);
      allconsumersSheet["!cols"] = [
        { wch: 5 },   // #
        { wch: 25 },  // Ism
        { wch: 20 },  // Sinf
        { wch: 8 },   // Yosh
      ...
      XLSX.utils.book_append_sheet(workbook, allconsumersSheet, "Barcha Istamolchilar");

      // Har bir sinf uchun alohida sheet
      Object.keys(consumersByClass).sort().forEach((className) => {
        const classconsumers = consumersByClass[className];
        const classData = [
          [`${className} - Istamolchilar ro'yxati`],
          [`Jami: ${classconsumers.length} ta Istamolchi`],
          [""],
          ["#", "Ism Familiya", "Yosh", "Telefon", "Email", "KELDI", "KETDI"]
        ];

        classconsumers.forEach((student, idx) => {
          classData.push([
            idx + 1,
            student.name || "-",
            student.age || "-",
  ...
  const activeCount = filteredconsumers.filter(
    (s) => s.status === "active"
  ).length;
  ...
  const davomat = consumers.length > 0 ? Math.round((kelganlar / consumers.length) * 100) : 0;
  ...
  return (
    <div className="px-6 py-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Istamolchilar</h1>
          <p className="text-sm text-gray-500 mt-1">Istamolchilarni boshqarish va ro'yxatdan o'tkazish</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-5 mb-6">
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
          ...
          <p className="text-sm text-gray-500 mt-1">Jami Istamolchilar</p>
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100">
            <span className="text-xs text-gray-500">Ro'yxatda:</span>
            <span className="text-sm font-semibold text-blue-600">{consumers.length} ta</span>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs font-medium px-2 py-1 rounded-full bg-orange-50 text-orange-700">
              {consumers.length > 0 ? Math.round((consumers.filter(s => s.class.startsWith("9")).length / consumers.length) * 100) : 0}%
            </span>
          </div>
          <p className="text-2xl font-bold text-gray-900">{consumers.filter(s => s.class.startsWith("9")).length}</p>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs font-medium px-2 py-1 rounded-full bg-violet-50 text-violet-700">
              {consumers.length > 0 ? Math.round((consumers.filter(s => s.class.startsWith("10") || s.class.startsWith("11")).length / consumers.length) * 100) : 0}%
            </span>
          </div>
          <p className="text-2xl font-bold text-gray-900">
            {consumers.filter(s => s.class.startsWith("10") || s.class.startsWith("11")).length}
          </p>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="relative group">
              ...
              <input
                ...
                className="pl-11 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all w-64 font-medium"
              />
            </div>
            <select
              value={selectedClass}
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500">
            ...
            {filteredconsumers.length} ta Istamolchi
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paginatedconsumers.map((student, index) => (
                <tr
                  key={student.id}
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="text-sm text-gray-500">
              {startIndex + 1}-{Math.min(endIndex, filteredconsumers.length)} / {filteredconsumers.length} ta
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <button
      ...
      {filteredconsumers.length === 0 && (
        <div className="text-center py-12 bg-white rounded-lg border border-gray-200 mt-6">
          ...
          <p className="text-gray-500">Istamolchi topilmadi</p>
        </div>
      )}
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-gray-900">
                {modalMode === "add"
                  ? "Yangi Istamolchi Qo'shish"
                  : "Istamolchini Tahrirlash"}
              </h2>
              <button
